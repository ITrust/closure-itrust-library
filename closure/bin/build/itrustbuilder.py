...
def _GetOptionsParser():
  ...
  parser.add_option('--modules_info',
                    dest='modules_info',
                    action='store',
                    help=('Additional flag to define modules architecture, '
                          'defined in a js file'))
  parser.add_option('--modules_info_root',
                    dest='modules_info_root',
                    action='store',
                    default='',
                    help=('Additional flag to defined the root of paths given in '
                          'modules info file'))
  parser.add_option('--modules_info_uri_key',
                    dest='modules_info_uri_key',
                    action='store',
                    default='MODULES_URIS',
                    help=('Additional flag to defined the URI key in the modules '
                           'info file'))
  parser.add_option('--modules_info_deps_key',
                    dest='modules_info_deps_key',
                    action='store',
                    default='MODULES_INFO',
                    help=('Additional flag to defined the dependencies key in '
                          'the modules info file'))
  parser.add_option('--use_renaming_map',
                    dest='use_renaming_map',
                    action='store',
                    help=('Additional flag to defined a renaming map file'
                          'to use in the case of module compilation.'))
...
def main():
  ...
  # Retrieve modules informations in a module info file is specified
  if options.modules_info:
    modules_info_file = open(options.modules_info)
    file_data = modules_info_file.read()
    json_data = file_data[file_data.find('{'):file_data.rfind('}') + 1]
    modules = json.JSONDecoder(object_pairs_hook=collections.OrderedDict).decode(json_data)
    for keys in modules[options.modules_info_uri_key]:
      paths = modules[options.modules_info_uri_key][keys]
      if isinstance(paths, list):
        for path in paths:
          options.inputs.append(options.modules_info_root + path)
      else:
          options.inputs.append(options.modules_info_root + paths)
  ...
  if output_mode == 'list':
    ...
    jsfiles = [js_source.GetPath() for js_source in deps]
    if options.use_renaming_map:
        jsfiles.insert(0, options.use_renaming_map)

     # Build the compiler flags
    if options.modules_info:
      # Dict containing {filePath:moduleName}
      filesToFlag = collections.OrderedDict()
      # Dict containing {moduleName:dependencies}
      moduleDependencies = collections.OrderedDict()

      for key in modules[options.modules_info_uri_key]:
        paths = modules[options.modules_info_uri_key][key]

        if isinstance(paths, list):
          for path in paths:
            filesToFlag[options.modules_info_root + path] = key
        else:
          filesToFlag[options.modules_info_root + paths] = key

        for dep in modules[options.modules_info_deps_key][key]:
          if key in moduleDependencies:
            moduleDependencies[key].append(dep)
          else:
            moduleDependencies[key] = [dep]

      # Used to count the number of files owned by a module
      countFile = 0
      for jsfile in jsfiles:
        countFile = countFile + 1

        if jsfile in filesToFlag:
          # Build the --module flag
          modulename = filesToFlag[jsfile]
          moduleFlag = '--module=' + modulename + ':' + str(countFile)
          # Consider dependencies
          if modulename in moduleDependencies:
            for dep in moduleDependencies[modulename]:
              moduleFlag = moduleFlag + ':' + dep

          options.compiler_flags.append(moduleFlag)
          countFile = 0
